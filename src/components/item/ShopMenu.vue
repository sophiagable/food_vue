<template>
    <div class="card border-primary m-2">
        ...
        <div class="card-header">
            CURRY WU 日式咖哩飯
            <small class="text-primary">[午晚餐]</small>
        </div>
        <div class="card-body">
          <div class="info text-danger">
            <small class="info-word">*只點一份，請點 「+1」，若有特殊要求，
              請點<i class="order-edit material-icons ml-1"  title="詳細點餐">edit </i></small>
          </div>
            <div class="menu-category" v-for="cate in shopDetail" :key="cate.category_name">
                ...
                <div class="list-group mb-5">
                        <ul class="menu-item-list">
                          <li class="list-group-item" v-for="(item,idx) in cate.sub_list" :key="item.item_name">

                            <div class="row menu-item ">
                                <div class="col-9">
                                  <span @click.stop="orderDetailMenu(item)"></span>{{item.item_name}}
                                    <i class="order-edit material-icons"  title="詳細點餐" @click.stop="orderDetail(item.id)"> 
                                      edit
                                    </i>
                                    <i class="order-one material-icons align-middle" title="點一份" @click.stop="orderThis(item)">exposure_plus_1</i>
                                </div>
                            ...
                            <div class="row bg-light order-detail" v-if="item.id == editMeal">
                              <div class="close-detail"> <i class="material-icons text-dark" @click.stop="editMeal='';item.number='';item.note='';">close</i></div>
